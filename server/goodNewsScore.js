...
export const hasDisqualifyingSignal = (text, title = text) => severeNegativeHeadlinePatterns.some((pattern) => pattern.test(title)) ||
    [...tragedyPatterns, ...politicalPatterns, ...sportsPatterns].some((pattern) => pattern.test(text));
export const hasPositiveOutcomeSignal = (text) => outcomePatterns.some((pattern) => pattern.test(text));
export const scoreGoodNews = (text) => {
    let score = 0;
