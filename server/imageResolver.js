...
const decodeHtmlEntities = (value) => value
    ...
    .replace(/&lt;/gi, '<')
    .replace(/&gt;/gi, '>');
const looksLikeUtilityImage = (url) => {
...
const looksLikeSmallImageUrl = (url) => /(?:thumb|thumbnail|small|tiny|preview|\b(?:120|150|180|200|240|300|320|360|400)x(?:90|120|150|180|200|240|300|320|360|400)\b)/i.test(url);
export const normalizeImageUrl = (value, baseUrl) => {
    if (!value)
        return undefined;
    try {
...
const dimensionBonus = (width, height) => {
...
const addCandidate = (candidates, value, baseUrl, score) => {
...
const rankedUniqueUrls = (candidates) => {
...
const mediaCandidates = (value, baseUrl, baseScore) => {
    const entries = Array.isArray(value) ? value : value ? [value] : [];
    const candidates = [];
    for (const entry of entries) {
        addCandidate(candidates, entry?.$?.url ?? entry?.url, baseUrl, baseScore + dimensionBonus(entry?.$?.width, entry?.$?.height));
    }
    return candidates;
};
const imagesFromHtml = (html, baseUrl, baseScore) => {
    if (!html)
        return [];
    const candidates = [];
...
export const imageCandidatesFromFeedItem = (item, articleUrl) => {
    const candidates = [];
    ...
    candidates.push(...imagesFromHtml(item.contentEncoded ?? item.content ?? item.summary, articleUrl, 68));
...
export const imageFromFeedItem = (item, articleUrl) => imageCandidatesFromFeedItem(item, articleUrl)[0];
...
const imageCandidatesFromMetadataHtml = (html, articleUrl) => {
    const candidates = [];
    const metaTags = html.match(/<meta\b[^>]*>/gi) ?? [];
    const scoreByIdentity = {
    ...
    for (const tag of metaTags) {
        const identity = (attribute(tag, 'property') ?? attribute(tag, 'name') ?? '').toLowerCase();
...
const imageCandidatesFromArticleMetadata = async (articleUrl) => {
...
export const enrichArticleImages = async (articles) => {
    ...
    const worker = async () => {
        while (true) {
            ...
            const metadataCandidates = await imageCandidatesFromArticleMetadata(article.url);
